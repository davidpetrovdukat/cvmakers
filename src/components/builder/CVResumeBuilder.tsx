...
const BUILDER_COPY = {
  ...
  ja: {
    ...
    template: 'テンプレート',
    tokenActions: 'トークン操作',
    tokenActionsBody: (doc: string) => `${doc}を生成するにはトークンを使用してください。`,
    creatingDraft: '下書きを作成しています...',
    create: '作成（100 トークン）',
    creatingPdf: 'PDFを作成しています...',
    exportPdf: 'PDFを作成してエクスポート（150 トークン）',
    creatingDocx: 'DOCXを作成しています...',
    exportDocx: 'DOCXを作成してエクスポート（150 トークン）',
    aiAssist: 'AIアシスト',
    aiBody: '選択したセクションの書き直しや箇条書きのアイデア生成をAIに任せられます。',
    fillRequiredTitle: '先に必須項目をすべて入力してください',
    creatingPerfect: (doc: string) => `あなたに最適な${doc}を作成しています...`,
    improveAi: 'AIで改善（200 トークン）',
    fillRequired: 'AI改善を有効にするには、すべてのセクション（個人情報、要約、職務経歴、学歴、スキル）を入力してください。',
    sendingManager: '担当マネージャーに送信中です...',
    sendManager: '担当マネージャーに送信（800 トークン）',
    tips: 'ヒント',
    tipItems: [
      '職務経歴の箇条書きには動詞と数値を使いましょう。',
      '要約は3〜4文の明確な文章にまとめましょう。',
      '求人内容に合わせてキーワードを調整しましょう。',
    ],
    notices: {
      saveError: '下書きを保存できませんでした',
      downloadError: 'ファイルのダウンロードに失敗しました',
      saved: (tokens: number) => `下書きをダッシュボードに保存しました。${tokens}トークンを使用しました。`,
      exported: (format: string, tokens: number) => `${format.toUpperCase()}の準備ができました。${tokens}トークンを使用しました。`,
      exportError: (format: string) => `${format.toUpperCase()}のエクスポートに失敗しました`,
      aiError: '職務経歴書を改善できませんでした',
      aiDone: 'AIがドキュメントを改善し、PDFをダウンロードしました。',
      managerError: '担当マネージャーへの送信に失敗しました',
      managerPending: '数時間以内',
      managerDone: (doc: string, date: string) => `リクエストを送信しました。担当マネージャーが${date}までに改善した${doc}を共有します。`,
      managerQueued: '担当マネージャーにリクエストを送信しました。3〜6時間以内に更新があります。',
    ...
    forms: {
      ...
      website: 'ウェブサイト',
      websitePlaceholder: '個人サイト（任意）',
      linkedin: 'LinkedIn',
      photoUrl: '写真URL',
      photoUrlPlaceholder: '写真のリンク',
      profileAlt: 'プロフィール',
      removePhoto: '写真を削除',
      uploadPhoto: '写真をアップロード',
      professionalSummary: 'プロフェッショナルサマリー',
      summaryPlaceholder: 'ご経験と強みを要約してください',
      writingTips: '執筆のヒント',
      summaryTips: ['定量的な成果を使いましょう（パーセンテージ、売上、チーム規模など）。', '3〜4文の簡潔で明確な文章にまとめましょう。'],
      addRole: '役職を追加',
      title: '役職',
      ...
      endPlaceholder: '終了日または現在',
      bullets: '箇条書き（1行に1つ）',
      bulletsPlaceholder: '動詞 + 成果を使いましょう',
      remove: '削除',
      addEducation: '学歴を追加',
      degree: '学位',
      degreePlaceholder: '学位または資格',
      school: '学校',
      ...
      reviewName: '名前',
      reviewRole: '役職',
      reviewExperience: '職務経歴の件数',
      reviewEducation: '学歴の件数',
      reviewSkills: 'スキル',
      reviewHint: 'エクスポートボタンからPDFまたはDOCXファイルを生成できます。',
    },
  },
} as const;
